...
# Cuarto Semestre
# Víctor Manuel Ospina Bautista

.data
	array: .space 20
	true: .asciiz "Es palindrome."
	false: .asciiz "No es palindrome."
.text
	...
	loop:                                # Contains the loop to iterate through the array.
		beqz       $t2, estadoVerdadero        # If the iterator from [0] to [size - 1] is 0, then it is a palindrome.
		beqz       $t5, estadoVerdadero         # If the iterator from [size - 1] to [0] is 0, then it is a palindrome.
		...
		blt        $a1, 97, siguiente		 # If the first loaded character is not an alphabetic lower case, it will skip it.
		bgt        $a1, 122, siguiente        # If the first loaded character is not an alphabetic lower case, it will skip it.
		...
		bne        $a1, $a2, estadoFalso  # If both characters are different, the phrase is not a palindrome.
	...
	siguiente:                                # Skips the non-alphabetic lower case character from [0] to [size - 1].
